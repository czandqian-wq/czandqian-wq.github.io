...
test("keeps editable content separate and preserves key interactions", async () => {
  ...
  assert.match(glossary, /term: "Token"/);
  assert.match(glossary, /term: "多模态 Multimodal"/);
  assert.match(flipCard, /role="button"/);
  assert.match(flipCard, /tabIndex=\{0\}/);
  assert.match(flipCard, /event\.key === "Enter" \|\| event\.key === " "/);
  assert.match(flipCard, /aria-pressed=\{flipped\}/);
